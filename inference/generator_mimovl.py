...
MODEL_PATH = "/mnt/ckpts/MiMo-VL-7B-SFT/"
...
class DPODataGeneratorMiMoVL(DPODataGeneratorBase):
    def load_model(self):
    ...
    def postprocess_diff_cap_response(self, resp):
        ...
        if resp.startswith("**"):
            resp = resp[2:]
        if ("caption" in resp.lower()[:30]) and ": " in resp:
            resp = resp.split(": ", 1)[1].strip()
        if resp.startswith("**"):
            resp = resp[2:].strip()
        if resp.startswith("\\boxed{") and resp.endswith("}"):
            resp = resp[7:-1].strip()
        if resp.endswith("\""):
            resp = resp[:-1]
            if resp.startswith("\""):
                resp = resp[1:]
        resp = resp.replace("the video clip 2", "this video clip")
        resp = resp.replace("The video clip 2", "This video clip")
        resp = resp.replace("video clip 2", "this video clip")
        resp = resp.replace("Video clip 2", "This video clip")
        resp = resp.replace("the video clip 1", "the previous clip")
        resp = resp.replace("The video clip 1", "The previous clip")
